import re
import os
from IOHelper import create_directory, move_file

def label(input_path: str, syntaxChecker, debug: bool = False):
    print("Label the extracted methods and remove logs from the bodies.")
    for (dirpath, dirnames, filenames) in os.walk(input_path):
        for filename in filenames:
            ...
            if not (os.path.isfile(dirpath + '/filteredCode2Vec/' + filename + "_filteredCode2Vec.txt") and
                ...
                create_directory(dirpath + '/filteredCode2Vec/')
                create_directory(dirpath + '/filteredRNN/')
                ...
                for l in lines:
                    if (index == -1):
                        ...
                        index = -1;
                f.close()
                move_file(dirpath + "/" + filename + ".txt", dirpath + "/processed/" + filename + ".txt")
